...
class MainScreenView(context: Context) : ConstraintLayout(context){
	...
	private val methodsName = mapOf<String,String>("SUCCESS" to "onSuccessCallback",
		"ERROR" to "onErrorCallback", "CANCEL" to "onCancelCallback")
	...
	fun openFaceliveness(view: View, options: String?){
		...
		if(items.has("welcomeScreenConfig")){
			...
			var instructionList = defaultWelComeScreenConfig?.get("instructionPoint") as Array<*>
			...
			if(itemWelComeScreenConfig.has("extraInstructionPoint")){
				...
				val extraInstructionList = jsonArrayToArray(extraInstructionListObject) as Array<*>
	...
	// Old Arch Send Event from native to js
	/*fun onSendReactNativeEvent(type:String, data: WritableMap? = null) {
		...
		reactContext.getJSModule(RCTModernEventEmitter::class.java).receiveEvent(UIManagerHelper.getSurfaceId(reactContext),id, nameOfEvent, event)
	}*/
	...
	//new Arch Send Event from native to js
	...
	private fun combine(arr1: Array<*>, arr2: Array<*>): Array<Any> {
